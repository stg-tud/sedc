...
trait Boxes extends BoundedShapes with SolidShapes { this: Space with Points ⇒
  case class Box(p1: point,
                 p2: point) extends BoundedShape with SolidShape {
...
trait IntersectionShapes extends Shapes { this: Space with Points ⇒
  case class Intersection(s1: Shape,
                          s2: Shape) extends Shape {
...
trait UnionShapes extends SolidShapes with BoundedShapes { this: Space with Points ⇒
  abstract class Union[+S1 <: Shape, +S2 <: Shape] private[UnionShapes] (val shape1: S1, val shape2: S2) extends Shape {
  ...
  def Union[S1 <: Shape, S2 <: Shape](s1: S1, s2: S2) = (s1, s2) match {
    case (s1: SolidShape, s2: Shape) ⇒ new Union[s1.type, s2.type](s1, s2) {}
    case (s1: Shape, s2: SolidShape) ⇒ new Union[s1.type, s2.type](s1, s2) {}
...
object SpaceDemo extends App {
  ...
  //    def UnionAcrossSpaces(s: ExSpace)(s1: ExSpace#Shape, s2: ExSpace#Shape) = {}
  ...
  def UnionAcrossSpaces[S1 <: Shapes#Shape, S2 <: Shapes#Shape, ST <: ExSpace](s1: S1, s2: S2, target: ST)(implicit s1Tag: TypeTag[S1], s2Tag: TypeTag[S2], spaceTag: TypeTag[ST]) = {
    ...
    (s1Tag.tpe, s2Tag.tpe) match {
      case (TypeRef(p1, _, _), TypeRef(p2, _, _)) // if p1 =:= p2 && (p1.typeSymbol.toString() == spaceType.typeSymbol.toString()) ⇒ { // FIXME (!) We need a replacement for this strange "toString()" conversion...
      if (s1.space eq target) && (s2.space eq target) ⇒ {
        println("Both shapes are from the same space as the target space: "+p1+" <=> "+p2)
        // let's delegate this to the space's union method.
      }
